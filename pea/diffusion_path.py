...
# ===========================================================================
# (4) diffusion_forward : BAI TOAN NGUOC — khoi tu ANH GOC x, forward VP-noising,
#     KHONG chon reference nao. Baseline = diem cuoi thong ke cua forward process.
#
# Dong co: cac method truoc deu phai chon 1 reference (blur) roi di TOI x => nhay
# tham so blur (sigma, ksize, boundary). O day DAO CHIEU: khoi tu x, chay forward
# VP-SDE (them nhieu dan) di RA khoi x toi noise. Reference khong con duoc CHON —
# no la cai forward process HOI TU VE (Gaussian). Chi co x lam dau vao.
#
# Marginal VP analytic (KHONG train, KHONG score-net => lightweight):
#     x_t = sqrt(abar_t) * x  +  sqrt(1 - abar_t) * eps ,   eps ~ N(0, I)
# t: 0 (=> x, data) -> 1 (=> ~noise). Voi moi eps co 1 quy dao x -> x_T(eps).
#
# Completeness dang KY VONG (diem cuoi ngau nhien => baseline la PHAN BO):
#     sum_i E[phi_i] = f(x) - E[f(x_T)]
# => "baseline" = f tai noise, lay trung binh. KHONG fix 1 diem, KHONG tham so blur.
#
# Tich phan Ito left-point doc quy dao forward, ky vong tren P mau nhieu.
#   d_k = g_k . dgamma_k. LIG-measure mu_k ∝ |d_k| optional (dung tren path trung binh).
# Ngan sach: P * T = N grad eval. eps sinh 1 lan/quy dao (khong ton grad).
#
# t_max < 1 de khong pha het hoan toan (giu quy dao trong vung f con y nghia); mac
# dinh 0.9. Neu muon "di het" dat t_max=1.0.
# ===========================================================================
def diffusion_forward(x, grad_fn, N,
                      beta_min=0.1, beta_max=20.0,
                      P=4, t_max=0.9, use_lig=False,
                      gen=None, model=None, target=None, score="logit"):
    """
    Forward VP-noising tu ANH GOC x (bai toan nguoc, KHONG reference blur).

    x        : (3,H,W) da chuan hoa.
    grad_fn  : (M,3,H,W)->(M,3,H,W).
    N        : ngan sach grad; P*T = N.
    beta_min/max : lich VP (Song et al. continuous). Tham so DUY NHAT, bam DDPM chuan.
    P        : so mau nhieu lay ky vong (baseline = phan bo => can trung binh).
    t_max    : muc nhieu cuc dai (0..1). <1 => khong pha het (giu f y nghia).
    use_lig  : True => LIG-measure mu_k ∝ |d_k| tren path trung binh.
    gen      : torch.Generator cho eps (tai lap).
    model,target,score : completeness rescale ve f(x) - mean f(x_T).

    Tra ve attribution (3,H,W). Huong: dong gop khi PHA x ve noise, dao dau de doc
    nhu "dong gop cua feature vao f(x)" (sum ~ f(x) - E f(noise)).
    """
    device = x.device
    T = max(2, N // max(1, P))

    # luoi thoi gian t: 0 (x) -> t_max (noise). Chua endpoint 2 dau.
    tgrid = torch.linspace(0.0, t_max, T + 1, device=device)          # (T+1,)
    abar = _vp_abar(tgrid, beta_min, beta_max)                        # (T+1,)
    a_sqrt = abar.sqrt().view(-1, 1, 1, 1)                            # coef data
    b_sqrt = (1.0 - abar).clamp_min(0).sqrt().view(-1, 1, 1, 1)       # coef noise

    acc_gd = torch.zeros(T, *x.shape, device=device)                 # sum_P g_k*dgamma_k per step
    acc_dk = torch.zeros(T, device=device)                           # sum_P d_k per step
    fT_acc = torch.zeros((), device=device)                          # sum_P f(x_T)
    fvals_fn = _resolve_fvals(None, model, target, score, device)

    for _ in range(max(1, P)):
        with torch.no_grad():
            eps = torch.randn(x.shape, generator=gen, device=device)  # 1 mau nhieu/quy dao
            # quy dao forward: x_t = sqrt(abar)*x + sqrt(1-abar)*eps
            path = a_sqrt * x[None] + b_sqrt * eps[None]              # (T+1,3,H,W)
            path[0] = x                                              # neo dau = anh goc
        g = grad_fn(path[:-1])                                       # left-point Ito (T,3,H,W)
        with torch.no_grad():
            dgamma = path[1:] - path[:-1]
            gd = g * dgamma                                         # (T,3,H,W)
            acc_gd += gd
            acc_dk += gd.flatten(1).sum(dim=1)
            if fvals_fn is not None:
                fT_acc += fvals_fn(path[-1:][0][None]).reshape(())

    with torch.no_grad():
        gd_mean = acc_gd / max(1, P)                                # (T,3,H,W)
        if use_lig:
            mu = acc_dk.abs(); mu = mu / (mu.sum() + 1e-12)
            attr = (mu.view(-1, 1, 1, 1) * gd_mean).sum(dim=0)
        else:
            attr = gd_mean.sum(dim=0)                               # (3,H,W)

        # attr hien tinh doc chieu PHA (x -> noise): sum ~ f(noise)-f(x). Dao dau de
        # doc theo huong "dong gop vao f(x)" (nhat quan dau voi cac method reference->x).
        attr = -attr

        # completeness rescale: sum attr -> f(x) - E[f(x_T)]
        if fvals_fn is not None:
            fx = fvals_fn(x[None]).reshape(())
            fT_mean = fT_acc / max(1, P)
            total = (fx - fT_mean)
            attr = attr * (total / (attr.sum() + 1e-12))
    return attr